...
if MERGE_THUMB:
    OUT_DIR = Path("./results/mv_supcon_anchor_model_6cls")
else:
    OUT_DIR = Path("./results/mv_supcon_anchor_model_7cls")
...
train_ds = SupConAnchorDataset(train_df)
label_to_idx = train_ds.label_to_idx
idx_to_label = {v: k for k, v in label_to_idx.items()}

val_ds = SupConAnchorDataset(val_df, label_to_idx=label_to_idx)
test_ds = SupConAnchorDataset(test_df, label_to_idx=label_to_idx)
...
for epoch in range(1, EPOCHS + 1):
    ...
    for x_orig, cam0, cam1, cam2, cam3, y in train_loader:
        ...
        # stack all embeddings: (5B, H)
        features = torch.cat([z_orig, z0, z1, z2, z3], dim=0)

        # repeat labels 5 times: (5B,)
        labels = torch.cat([y, y, y, y, y], dim=0)
    ...
    if val_acc > best_val_acc + MIN_DELTA:
        ...
        torch.save({
            ...
            "config": {
                ...
                "model_type": "mv_supcon_anchor_single_view"
            }
        }, best_path)
...
save_confmat(
    ...
    "MV-SupCon Anchor - Salux Original Test"
)
