...
lines=["Consistency fine-tuning results","","CE-only Blender8 checkpoint warm-up. Cosine-consistency fine-tuning uses lr=1e-4. Selection uses Salux validation all9 accuracy.",""]
for _,x in agg.iterrows():
 e=ens[ens["lambda"]==x["lambda"]].iloc[0];lines.append(f"lambda {x['lambda']:g}: val all9 {100*x.val_all9_mean:.2f} +/- {100*x.val_all9_std:.2f}, DrOh {100*x.droh_accuracy_mean:.2f} +/- {100*x.droh_accuracy_std:.2f}, macro F1 {100*x.droh_macro_f1_mean:.2f} +/- {100*x.droh_macro_f1_std:.2f}, ensemble {100*e.accuracy:.2f} ({int(e.correct)}/675)")
lines += ["",f"Validation-selected lambda: {selected:g}.",f"Selected consistency ensemble versus CE-only ensemble: {comparison['consistency_accuracy']*100:.2f} percent vs {comparison['ce_accuracy']*100:.2f} percent ({comparison['difference_pp']:+.2f} pp), p={comparison['mcnemar_p']:.4g}, CI [{comparison['bootstrap_95ci_pp'][0]:.2f}, {comparison['bootstrap_95ci_pp'][1]:.2f}] pp."]
